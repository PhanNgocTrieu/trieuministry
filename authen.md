...
Firebase Authentication: We will enable Anonymous Auth for users (no password needed, auto-login) and Email/Password for Admin. You will need to enable these Sign-in providers in your Firebase Console.
...
read: allow all.
create
: allow if valid content & authenticated (anon or real).
update
: allow if auth.uid == resource.data.userId OR auth.token.admin == true.
delete
: allow ONLY if auth.token.admin == true (or specific admin email hardcoded).
Verification Plan
Automated Tests
