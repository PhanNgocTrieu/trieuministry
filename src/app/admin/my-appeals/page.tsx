    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">My Ministry Appeals</h1>
                    <p className="text-slate-500 dark:text-slate-400">Track the status of your personal fundraising requests.</p>
                </div>
                <Link 
                    href="/donate" 
                    className="bg-blue-600 text-white px-5 py-2.5 rounded-xl hover:bg-blue-500 shadow-lg shadow-blue-500/20 font-bold flex items-center gap-2"
                >
                            ...
                            {appeals.length > 0 ? (
                                appeals.map((appeal) => (
                                    <tr key={appeal.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="font-bold text-slate-900 dark:text-white">{appeal.title}</div>
                                            <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 line-clamp-1">{appeal.content}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="space-y-1 min-w-[150px]">
                                                ...
                                                <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-1.5 border border-slate-300 dark:border-white/5">
                                                    <div 
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold capitalize border
                                                ${appeal.status === 'published' ? 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20' : 
                                                  ...
                                                  appeal.status === 'completed' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20' :
                                ...
                                <tr>
                                    <td colSpan={4} className="px-6 py-12 text-center text-slate-500">
                                        ...
                                        <Link href="/donate" className="inline-block mt-4 text-blue-500 hover:text-blue-400 text-sm font-bold hover:underline">
                                            Submit a Request
                                        </Link>
                                    </td>
                                </tr>
