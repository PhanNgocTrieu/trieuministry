...
const Navbar = () => {
  ...
  const { 
   ...
   const isActive = (path: string) => pathname === path 
     ? 'text-purple-600 dark:text-purple-400 font-bold bg-purple-50 dark:bg-white/5 rounded-lg' 
     : 'text-slate-600 dark:text-slate-300 hover:text-purple-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg';
   ...
   return (
     <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${scrolled ? 'bg-white/80 dark:bg-slate-950/80 backdrop-blur-md border-slate-200 dark:border-white/5 py-2 shadow-sm dark:shadow-none' : 'bg-transparent border-transparent py-4'}`}>
             ...
             {user ? (
                <div className="relative" ref={dropdownRef}>
                   ...
                   >
                      ...
                      <span className="text-sm font-semibold max-w-[100px] truncate hidden xl:block text-slate-700 dark:text-slate-200">
                         {user.displayName?.split(' ')[0] || 'User'}
                      </span>
                      {isAdmin && (
                        <span className="hidden xl:inline-block px-1.5 py-0.5 bg-purple-100 dark:bg-purple-500/20 text-purple-600 dark:text-purple-300 border border-purple-200 dark:border-purple-500/30 text-[10px] font-bold uppercase rounded ml-1">{t('nav.admin_role')}</span>
                      )}
                      {isVolunteer && (
                        <span className="hidden xl:inline-block px-1.5 py-0.5 bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-300 border border-green-200 dark:border-green-500/30 text-[10px] font-bold uppercase rounded ml-1">Volunteer</span>
                      )}
                         ...
                         <div className="px-4 py-3 border-b border-slate-100 dark:border-white/5 mb-2 bg-slate-50 dark:bg-slate-800/50">
                            ...
                            <div className="flex items-center gap-1 mt-2">
                                ...
                                {isAdmin ? (
                                    ...
                                    <span className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600 text-[10px] font-bold uppercase rounded">{t('nav.user_role')}</span>
                                )}
                            </div>
                         </div>
